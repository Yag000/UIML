...
iE and iA are given box-free formulas as input.*)
       method iA s = catch_e (fun () -> s |> eval |> fail_on_modality |> isl_simplified_A v |> fail_on_modality |> string_of_formula)
       method iE s = catch_e (fun () -> s |> eval |> fail_on_modality |> isl_simplified_E v |> fail_on_modality |> string_of_formula)
